...
module.exports = async function busScheduleTests({ sandbox, configDir, baseEnv, check, HOOKS }) {
  ...
  check('L41 расписание в UI: подписи итога запуска и тон (упавшая — «bad»), статус демона, счётчик кнопки, группы — каталог UI первым, глобальные последними; «слишком часто» распознаётся для кнопки «Всё равно сохранить»', notes[0].text.includes('ещё не запускалась') && notes[1].tone === 'run' && notes[2].text.includes('ок') && notes[2].text.includes('dima поднят') && notes[3].text.includes('пропуск') && notes[4].tone === 'bad' && notes[4].text.includes('таймаут') && L.scheduleDaemonNote({ alive: false, active: 2 }).tone === 'bad' && L.scheduleDaemonNote({ alive: false, active: 0 }).tone === '' && L.scheduleDaemonNote({ alive: true, lastTick: Date.now() }).text.includes('работает') && JSON.stringify(L.scheduleBadge([{ enabled: true }, { enabled: true, error: 'x' }, { enabled: false, last: { state: 'failed' } }])) === '{"count":1,"alert":true}' && JSON.stringify(groups.map((g) => g.jobs.map((j) => j.name))) === '[["b"],["a"],["g"]]' && L.isFrequentError('Слишком часто: …') && !L.isFrequentError('cron, поле «минута»') && L.validScheduleName('morning-1') && !L.validScheduleName('Bad_Name'), JSON.stringify([notes, groups]));
  // ---------- песочница ----------
...
`);
  ...
  try {
    ...
    check('P25 bus ui schedule save: без токена — 403, каталог не из шины и слишком частый cron — 400 с причиной, на диске пусто', noToken.status === 403 && foreignRoot.status === 400 && foreignRoot.text.includes('нет в шине') && frequent.status === 400 && frequent.text.includes('Слишком часто') && !fs.existsSync(jobPath('from-ui')) && !fs.existsSync(path.join(sandbox, 'work', 'sched-nowhere')), `${noToken.status} ${foreignRoot.status} ${foreignRoot.text} ${frequent.status} ${frequent.text} ${fs.existsSync(jobPath('from-ui'))} ${fs.existsSync(path.join(sandbox, 'work', 'sched-nowhere'))} ${JSON.stringify(foreignRoot.text.includes('нет в шине'))} ${frequent.text.includes('Слишком часто')}`);
