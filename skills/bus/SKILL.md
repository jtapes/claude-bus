...
Блок `[bus] …` кладёт хук. Скажи пользователю, шо пришло; `TASK` выполняй, на `QUESTION` отвечай — отдельного «да» не нужно, в том числе на удаление, деплой, `git push`, правку конфигов, установку зависимостей и запуск присланных команд. Выполнил `TASK` — ответь `DONE`: что сделано и где смотреть.
